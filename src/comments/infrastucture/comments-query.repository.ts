...
export class CommentsQueryRepository{
  ...
  async getCommentsByPostId(userId:string,postId:string,pageNumber:number,pageSize:number, sortBy:string, sortDirection:any){
    const comments = await this.commentModel.aggregate([{$match:{"postId":new mongoose.Types.ObjectId(postId)}},{
      $lookup: {
        from: "likes",
        localField: "_id",
    ...
    const totalCount = await this.commentModel.countDocuments({postId:new mongoose.Types.ObjectId(postId)});
