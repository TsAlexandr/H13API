...
export class UserQueryRepository{
  ...
  async findByLogin(login:string){
    const user = await this.userModel.findOne({login:login}/*{$or:[{"email":loginOrEmail},{"userName":loginOrEmail}]}*/)
    return user;
  }
  ...
  async getUsers(searchLoginTerm:string, searchEmailTerm:string, pageNumber:number,pageSize:number, sortBy:string, sortDirection:any){
    ...
    const totalCount = await this.userModel.count({$or:[{"login":{$regex:searchLoginTerm, $options : 'i' }},{"email":{$regex: searchEmailTerm,$options : 'i' }}]});
  ...
  async getUserByRecoveryCode(code:string):Promise<any>{
    const user = await this.userModel.findOne({"recoveryData.recoveryCode":code})
    return user
  }
}